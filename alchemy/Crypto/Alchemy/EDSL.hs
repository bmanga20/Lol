...
class SymCT expr where
  ...
  addLit :: (AddPublicCtx t m m' zp zq)
...
data PT2CT              -- GHC wants complete kind signature for polykindedness
  ...
  :: *
  where
    P2CTerm  :: ctexpr (CT m zp (Cyc t (lookup m m'map) (zqs ! d)))
             -> PT2CT ctexpr m'map zqs d (Cyc t m zp)
...
instance (SymCT ctexpr) => SymPT (PT2CT ctexpr) where
  ...
  (P2CTerm a) +# (P2CTerm b) =
    P2CTerm $ a + b \\ witness entailRingSymCT a
  (P2CTerm a) +# (P2CLit b) = P2CTerm $ addPublicCT b a
  (P2CLit a) +# (P2CTerm b) = P2CTerm $ addPublicCT a b
