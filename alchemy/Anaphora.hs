...
{-# LANGUAGE TypeFamilies, FlexibleContexts, RankNTypes, PartialTypeSignatures, ScopedTypeVariables #-}
...
{-
exdiv3 :: (AppLiftable i, SSym repr, LamPure repr, AssertPos repr,
     SymDIV repr) ⇒
...
exdiv3' :: (AppLiftable i, SSym repr, LamPure repr,
     Extends (IO :. i1) (IO :. i), SymDIV repr, AssertPos repr) =>
    i1 (repr Int) -> (:.) IO i (repr (Int -> Int))

(k :. l) -> m :. (k :. l) = IO :. i
i ~ j

exdiv3' y = assert_locus' $ \locus ->
     lam (\(x :: (Extends (IO :. j) (IO :. i), j ~ k :. l) => j (repr Int)) ->
       guarded_div3' locus (vr x :: (IO :. i) (repr Int)) (vr y) :: (IO :. i) (repr Int))
-}
{-
...
(vr x)                                 :: (IO :. i) (repr Int)
(vr y)                                 :: m (repr Int)
guarded_div3' locus (vr x) (vr y)      :: (IO :. i) (repr Int)

x :: j (repr a)
(vr x) :: (m :. k) (repr a) where Extends (m :. j) (m :. k)

=> (IO :. i) ~ (m :. k)
=> m ~ IO
=> i ~ k

lam $ \x -> f x :: (m :. r) (repr (a -> b))
=> x :: (forall s . (r :. s) (repr a))
=> f x :: m :. (r :. s) (repr b)
i ~ k ~ r :. s
-}

exdiv3Top :: (AppLiftable i, SSym repr, LamPure repr, AssertPos repr,
     SymDIV repr) =>
    (:.) IO i (repr (Int -> Int -> Int))
exdiv3Top = lam exdiv3'

exdiv3' :: forall i repr j . (AppLiftable i, SSym repr, LamPure repr,
     Extends (IO :. j) (IO :. i), SymDIV repr, AssertPos repr) =>
    j (repr Int) -> (IO :. i) (repr (Int -> Int))
-- EAC: can't write this point-free?
exdiv3' y = lam $ exdiv3'' y

exdiv3'' :: forall i repr j k . (AppLiftable i, SSym repr, LamPure repr,
     Extends (IO :. j) (IO :. i), SymDIV repr, AssertPos repr, Applicative k) =>
    j (repr Int) -> (i :. k) (repr Int) -> (IO :. (i :. k)) (repr Int)
exdiv3'' y x = guarded_div3' (vr x) (vr y :: (IO :. i) (repr Int))

guarded_div3' :: (m' ~ (IO :. i), Applicative i, SSym repr,
                 Extends m m', SymDIV repr) =>
     m' (repr Int) -> m (repr Int) -> m' (repr Int)
guarded_div3' x y = x /: weakens y
...
{-
an2'
  ∷ (Applicative i, SSym repr) ⇒ i (repr Int) → (:.) IO i (repr Int)
an2' = \x -> with_ref (\r -> (note_ref r (var x)) +: get_ref r)
-}
{-
an2
  ∷ ∀ {repr ∷ ★ → ★} {i ∷ ★ → ★}.
    (AppLiftable i, SSym repr, LamPure repr) ⇒
    (:.) IO i (repr (Int → Int))
...
/home/oleg/papers/MetaFX/hybrid/Anaphora.hs:95:41:
    ...
    In the second argument of â€˜writeIORefâ€™, namely â€˜yâ€™
