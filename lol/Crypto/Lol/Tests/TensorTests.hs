...
prop_fmapT :: forall t m r . _ => t m r -> Test '(t,m,r)
prop_fmapT x = test $ fmapT id x == x \\ entailEqT @t @m @r \\ entailIndexT @t @m

prop_fmap :: forall t m r . _ => t m r -> Test '(t,m,r)
prop_fmap x = test $ (fmap id x) == x \\ (entailEqT @t @m @r) \\ entailIndexT @t @m

-- divG . mulG == id in Pow basis
prop_ginv_pow :: forall t m r . _ => t m r -> Test '(t,m,r)
prop_ginv_pow x = test $ (fromMaybe (error "could not divide by G in prop_ginv_pow") $
  divGPow $ mulGPow x) == x \\ entailEqT @t @m @r

-- divG . mulG == id in Dec basis
prop_ginv_dec :: forall t m r . _ => t m r -> Test '(t,m,r)
prop_ginv_dec x = test $ (fromMaybe (error "could not divide by G in prop_ginv_dec") $
  divGDec $ mulGDec x) == x \\ entailEqT @t @m @r

-- divG . mulG == id in CRT basis
prop_ginv_crt :: forall t m r . _ => t m r -> Test '(t,m,r)
prop_ginv_crt x = test $ fromMaybe (error "no CRT in prop_ginv_crt") $ do
  divGCRT' <- divGCRT
  mulGCRT' <- mulGCRT
  return $ (divGCRT' $ mulGCRT' x) == x \\ entailEqT @t @m @r

-- mulGDec == lInv. mulGPow . l
prop_g_dec :: forall t m r . _ => t m r -> Test '(t,m,r)
prop_g_dec x = test $ (mulGDec x) == (lInv $ mulGPow $ l x) \\ entailEqT @t @m @r

prop_g_crt :: forall t m r . _ => t m r -> Test '(t,m,r)
prop_g_crt x = test $ fromMaybe (error "no CRT in prop_g_crt") $ do
  mulGCRT' <- mulGCRT
  crt' <- crt
  crtInv' <- crtInv
  return $ (mulGCRT' x) == (crt' $ mulGPow $ crtInv' x) \\ entailEqT @t @m @r

-- crtInv . crt == id
prop_crt_inv :: forall t m r . _ => t m r -> Test '(t,m,r)
prop_crt_inv x = test $ fromMaybe (error "no CRT in prop_crt_inv") $ do
  crt' <- crt
  crtInv' <- crtInv
  return $ (crtInv' $ crt' x) == x \\ entailEqT @t @m @r

-- lInv . l == id
prop_l_inv :: forall t m r . _ => t m r -> Test '(t,m,r)
prop_l_inv x = test $ (lInv $ l x) == x \\ entailEqT @t @m @r
...
prop_trem_pow x = test $ (twacePowDec $ (embedPow x :: t m' r)) == x \\ entailEqT @t @m @r

-- tests that twace . embed == id in the Dec basis
prop_trem_dec :: forall t m m' r . (Fact m, Fact m', _) => t m r -> Test '(t,m,m',r)
prop_trem_dec x = test $ (twacePowDec $ (embedDec x :: t m' r)) == x \\ entailEqT @t @m @r
...
prop_trem_crt x = test $ fromMaybe (error "no CRT in prop_trem_crt") $
  (x==) <$> (twaceCRT <*> (embedCRT <*> pure x :: Maybe (t m' r))) \\ entailEqT @t @m @r
...
prop_twEmID x = test $
  ...
  ((embedPow x) == x) &&
  ((embedDec x) == x) &&
  (((fromMaybe (error "twemid_crt") embedCRT) x) == x) \\ entailEqT @t @m @r
...
prop_twace_invar1_pow = test $ fromMaybe (error "could not divide by G in prop_twace_invar1_pow") $ do
  ...
  return $ (twacePowDec input) == output \\ entailEqT @t @m @r
...
prop_twace_invar1_crt = test $ fromMaybe (error "no CRT in prop_twace_invar1_crt") $ do
  ...
  return $ (twaceCRT' input) == output \\ entailEqT @t @m @r
...
prop_twace_invar2_powdec = test $
  ...
  in (twacePowDec input) == output \\ entailEqT @t @m @r
